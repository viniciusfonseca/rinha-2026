...
static bool lb_backend_fd_healthy(int fd) {
    char byte = 0;
    ssize_t rc = recv(fd, &byte, 1, MSG_PEEK | MSG_DONTWAIT);
    if (rc > 0) {
        return true;
    }
    if (rc == 0) {
...
static bool lb_try_acquire_backend_from_pool(lb_backend_pool_t *pool, lb_session_t *session) {
    ...
    for (size_t i = 0; i < LB_BACKEND_POOL_SIZE_MAX; i++) {
        ...
        if (!lb_backend_fd_healthy(slot->fd)) {
            close(slot->fd);
            slot->fd = -1;
            slot->ready = false;
            slot->pending = false;
...
static int lb_finish_response_write(
    ...
    session->buffer_sent[1] = 0u;
    lb_release_backend_after_response(session, backend_pools);
